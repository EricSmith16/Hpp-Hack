...
void cOffset::GetRenderType()
{
	...
	DWORD D3Dll = (DWORD)GetModuleHandleA( D3DDIM_DLL );
	DWORD GlDll = (DWORD)GetModuleHandleA( OPENGL32_DLL );

	if ( HwDll || ( GlDll && HlMod && !D3Dll && !SwDll ) )
		HLType = RENDERTYPE_HARDWARE;
	else if ( SwDll || ( HlMod && !D3Dll && !HwDll ) )
...
DWORD cOffset::FindClientTable()
{
	...
	if ( PatternAddress )
	{
		DWORD ClientTablePtr = *(PDWORD)( FindReference( HwBase , HwEnd , PatternAddress ) + 0x13 );
...
DWORD cOffset::FindUserMsgBase()
{
	...
	DWORD UserMsgBase = Absolute( FindPattern( (PCHAR)Pattern_UserMsg , OFF_MSG_BASE_MASK1 , Address , Address + 0x32 , 3 ) );
	...
	UserMsgBase = FindPattern( (PCHAR)Pattern_UserMsg2 , OFF_MSG_BASE_MASK2 , UserMsgBase , UserMsgBase + 0x32 , 7 );
...
void cOffset::SVCMessagesInitaizie()
{
